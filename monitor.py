...
class Monitor:
    ...
    def _average_total_distance(self) -> float:
        ...
        tot_dist = 0
        cnt = 0
        val = list(self._activities[DRIVER].values())
        for act in val:
            cnt += 1
            for i in range(len(act)):
                if len(act) >= 2 and i != len(act) - 1:
                    tot_dist += location.manhattan_distance(act[i].location,
                                                            act[i + 1].location)
        return tot_dist / cnt
    ...
    def _average_ride_distance(self) -> float:
        ...
        for act in val:
            cnt += 1
            for i in range(len(act)):
                if len(act) >= 2 and i != len(act) - 1 and act[i].description \
                        == PICKUP and act[i + 1].description == DROPOFF:
                    tot_dist += location.manhattan_distance(act[i].location,
                                                            act[i + 1].location)
        return tot_dist / cnt
